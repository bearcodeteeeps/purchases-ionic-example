...
export class HomePage {
  constructor(public platform: Platform, public purchases: Purchases) {
    platform.ready().then(() => {
      ...
      this.purchases
        ...
        .subscribe(
          ...
          error =>
            console.log(
              "PURCHASES updatedPurchaserInfoReceived " + JSON.stringify(error)
            )
        );
      ...
      this.purchases
        .identify("purchases_sample_id_4")
        .subscribe(
          info => console.log("PURCHASES identify " + JSON.stringify(info)),
          error => console.log("PURCHASES identify " + JSON.stringify(error))
        );

      this.purchases
        .getProducts(["onemonth_freetrial"])
        .subscribe(
          products =>
            console.log(
              "PURCHASES products fetched " + JSON.stringify(products)
            ),
          error =>
      ...
      this.purchases
        .getAppUserID()
        .then(appUserID => console.log("PURCHASES appUserID " + appUserID));

      this.purchases
        .createAlias("newAppUserID")
        .subscribe(
          purchaserInfo =>
            console.log(
              "PURCHASES createAlias " + JSON.stringify(purchaserInfo)
            ),
          error => console.log("PURCHASES createAlias " + JSON.stringify(error))
        );
    });
  }
  ...
  reset() {
    this.purchases
      .reset()
      .subscribe(
        info => console.log("PURCHASES reset " + JSON.stringify(info)),
        error => console.log("PURCHASES reset " + JSON.stringify(error))
      );
  }
}
